...
fixture_root=${1:-$project_dir/sandbox/method-combos/random_samples}
...
failures=0
printf '%-18s %s\n' Sample Result
for fixture in "$fixture_root"/mask*-Dataset*; do
  test -d "$fixture" || continue
  sample=$(basename "$fixture")
  mask_part=${sample%%-*}
  mask=${mask_part#mask}
  name=${sample#*-}
  set +e
...
if ((failures != 0)); then
  printf '%d sampled combination(s) differ.\n' "$failures" >&2
  exit 1
fi
